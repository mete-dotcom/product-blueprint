 ...
 * /adauto/activate
 *
 * Web-based activation for the ADAUTO CLI.
 * Flow:
...
export default function AdautoActivate() {
  ...
  async function handleAuth(e: React.FormEvent) {
    ...
    const r = await fetch("/api/adauto/activate-cli", {
      method: "POST",
    ...
    if (data.success) {
      ...
      if (cliMode.current) {
        await fetch("/api/adauto/activate-session", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  function pollForCLI() {
    ...
    const iv = setInterval(async () => {
      ...
      try {
        const r = await fetch(`/api/adauto/session?id=${id}`);
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 mb-2">
              <div className="w-8 h-8 rounded-lg bg-amber-600 flex items-center justify-center text-white font-bold text-sm">A</div>
              <span className="text-white font-semibold text-xl">ADAUTO</span>
            </div>
            {cliMode.current && (
              <span className="block text-xs text-amber-400 bg-amber-950 border border-amber-800 rounded-full px-3 py-1 inline-block mt-1">
            ...
            <div className="bg-[#111118] border border-white/10 rounded-2xl p-6">
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                <input
                  ...
                  className="w-full bg-[#1a1a25] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-amber-500"
                  required
                />
                <input
              ...
              <p className="text-center text-slate-500 text-xs mt-4">
                No account?{" "}
                <a href="/register" className="text-amber-400 hover:text-amber-300">Create one free</a>
              </p>
            </div>
          )}
          ...
          {stage === "checkout" && (
            <div className="bg-[#111118] border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-slate-400 text-sm mb-6">
                Free tier includes 1 campaign, 3 posts/day. Pro unlocks unlimited everything.
              </p>
                ...
                {([
                  {
                    label: "Free",
                    price: "$0/mo",
                    desc:  "1 campaign · 3 posts/day · 1 platform",
                    id:    "",
                  },
                  {
                    label: "Pro",
                  ...
                  <button
                    ...
                    className="w-full text-left bg-[#1a1a25] hover:bg-[#22222f] border border-white/10 hover:border-amber-500/50 rounded-xl p-4 transition-all group"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-white font-medium">{plan.label}</span>
                      <span className="text-amber-400 font-semibold">{plan.price}</span>
                    </div>
              ...
              <p className="text-center mt-4">
                <a href="/adauto/pricing" className="text-amber-400 hover:text-amber-300 text-xs">
                  See full pricing →
                </a>
              </p>
            </div>
            ...
            <div className="bg-[#111118] border border-emerald-800/40 rounded-2xl p-8 text-center">
              ...
              {!cliMode.current && (
                <p className="text-slate-400 text-sm mt-4">
                  Run <code className="bg-slate-800 px-2 py-0.5 rounded text-amber-300">adauto run --campaign my-first</code> to start
                </p>
              )}
            </div>
          )}
          ...
          {stage === "error" && (
            <div className="bg-[#111118] border border-red-800/40 rounded-2xl p-8 text-center">
              ...
              <p className="text-slate-400 text-sm mb-4">Run <code className="bg-slate-800 px-2 py-0.5 rounded">adauto activate</code> again</p>
              <button onClick={() => setStage("auth")} className="text-amber-400 hover:text-amber-300 text-sm">
                Try again
              </button>
            </div>
          )}
