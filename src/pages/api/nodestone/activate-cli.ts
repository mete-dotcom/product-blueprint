/**
 * POST /api/nodestone/activate-cli
 *
 * Direct CLI activation: email + password → nodestone license payload.
 * Used by `nodestone init --email you@example.com`.
 */

import type { NextApiRequest, NextApiResponse } from "next";
import crypto from "crypto";
import { getUser, getNodestoneLicense } from "../../../lib/store";
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `[NODESTONE] Activation — ${tier}`,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Response>) {
  ...
  const lic = await getNodestoneLicense(normalizedEmail);
  if (!lic) {
    return res.status(402).json({
      success:     false,
      error:       "no active nodestone subscription",
      upgrade_url: pricingUrl("nodestone"),
    });
  }
  ...
  if (new Date(lic.expires_at) < new Date()) {
    return res.status(402).json({
      success:     false,
      error:       "nodestone subscription expired",
      upgrade_url: pricingUrl("nodestone"),
    });
  }
