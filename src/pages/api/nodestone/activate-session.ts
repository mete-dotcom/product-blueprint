/**
 * POST /api/nodestone/activate-session
 * { email, password, session }
 *
 * Called by the web UI after login: stores the license in KV so the
 * CLI polling loop at /api/nodestone/session can pick it up immediately.
 */

import type { NextApiRequest, NextApiResponse } from "next";
import crypto from "crypto";
import { getUser, getNodestoneLicense, setNodestoneActivation } from "../../../lib/store";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Response>) {
  ...
  await setNodestoneActivation(String(session), lic, 7200);
