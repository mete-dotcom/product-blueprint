/**
 * POST /api/adauto/redeem-recovery-key
 *
 * CLI endpoint: exchange a one-time adauto recovery key for a license.
 * Called by `adauto activate --recovery-key RK1-AA-... --email user@x.com`.
 *
...
export default async function handler(
  ...
  const lic = await getAdautoLicense(result.payload.email.toLowerCase());
  if (!lic) {
