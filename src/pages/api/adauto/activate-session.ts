/**
 * POST /api/adauto/activate-session
 * { email, password, session }
 *
 * Called by the web UI after login: stores the license in KV so the
 * CLI polling loop at /api/adauto/session can pick it up immediately.
 */

import type { NextApiRequest, NextApiResponse } from "next";
import crypto from "crypto";
import { getUser, getAdautoLicense, setAdautoActivation } from "../../../lib/store";
...
function hashPassword(password: string): string {
  return crypto
    .createHash("sha256")
    .update(password + (process.env.DEEPSTRAIN_SECRET_KEY || "salt"))
    .digest("hex");
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Response>) {
  ...
  await setAdautoActivation(String(session), lic, 7200);
