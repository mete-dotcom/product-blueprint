/**
 * POST /api/adauto/issue-recovery-key
 *
 * Admin-only: issue a single-use recovery key for an adauto customer whose
 ...
 *
 * Customer runs:
 *   adauto activate --recovery-key "RK1-AA-..." --email user@x.com
 */

import type { NextApiRequest, NextApiResponse } from "next";
import { issueRecoveryKey, verifyAdminKey } from "../../../lib/recovery-key";
import { getAdautoLicense } from "../../../lib/store";
...
export default async function handler(
  ...
  const lic = await getAdautoLicense(normalizedEmail);
  if (!lic) {
    return res.status(404).json({
      error: `No adauto license found for ${normalizedEmail}.`,
    });
  }
  ...
  return res.status(200).json({
    ...
    instructions:
      `adauto activate --recovery-key "${payload.token}" --email "${normalizedEmail}"\n` +
