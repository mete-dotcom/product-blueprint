/**
 * POST /api/atlas/activate-session
 * { email, password, session }
 *
 * Called by the web UI after login: stores the license in KV so the
 * CLI polling loop at /api/atlas/session can pick it up.
 */

import type { NextApiRequest, NextApiResponse } from "next";
import crypto from "crypto";
import { getUser, getAtlasLicense, setAtlasActivation } from "../../../lib/store";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Response>) {
  ...
  await setAtlasActivation(String(session), lic, 7200);
